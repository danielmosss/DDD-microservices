...
func (r *PostgresMetingRepository) Save(ctx context.Context, m models.Meting) (models.Meting, error) {
	query := `
		...
		RETURNING time, id, sensor_id, kunstwerk_id, waarde, is_afwijking, is_handmatig, inspectie_id, afgehandeld
	`
	...
	).Scan(&outTime, &outID, &outSensorID, &outKunstwerkID, &outWaarde, &outIsAfwijking, &outIsHandmatig, &outInspectieID, &outAfgehandeld)
	...
	saved.IsAfwijking = outIsAfwijking
